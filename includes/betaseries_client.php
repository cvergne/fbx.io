...
class BetaSeries_Client
{
     ...
     *
     * @var BetaSeries_Client::JSON or BetaSeries_Client::XML
     */
    protected $format;
     ...
     */
    public function __construct($baseUrl, $apikey, $format = BetaSeries_Client::JSON, $language = BetaSeries_Client::LANGUAGE_VO)
    {
    ...
    {
        $s = json_decode($this->search($title), true);
        if ($s['root']['code'] == 1 && count($s['root']['shows']) > 0) {
            return $s['root']['shows'][0]['url'];
        }
        else {
            return false;
        }
    ...
    public function getOauth($login)
    {
        return $this->fetch('/members/oauth', array('login' => $login));
    }
     ...
     * @return string
     */
    public function getSubtitles($slug, $season = null, $episode = null, $file = null)
    {
        $parameters = array();
     ...
     **/
    protected function fetch($url, $params = array(), $method = BetaSeries_Client::GET)
    {
        $params = array_merge($params, array('key' => $this->apiKey));

        $query = '';
        if ($method == BetaSeries_Client::GET) {
        ...
        if ($method == BetaSeries_Client::POST) {
